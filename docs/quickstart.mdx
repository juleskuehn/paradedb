...
Next, let's see how ParadeDB handles a search query like "bluetooth speaker", which

```sql
SELECT description, rating, category
FROM mock_items
WHERE mock_items @@@ 'description:"bluetooth speaker"~1';
```
...
Notice that we must wrap phrases with whitespace in double quotes and our use of the `~1` slop operator,
which tells the search engine to return matches even if they are separated by `1` word.

Finally, let's implement typo tolerance using the `ngrams` tokenizer. To do this, we'll need to drop and
recreate our index:
...
USING bm25 ((mock_items.*))
WITH (
  text_fields='{"description": {tokenizer: {type: "ngram", min_gram: 3, max_gram: 3, prefix_only: true}}, "category": {}}'
);

SELECT description, rating, category
FROM mock_items
WHERE mock_items @@@ 'description:bluetoohth';
```
...
This quickstart covers only a small subset of the queries you can run with [ParadeQL, our mini query language](search/bm25).
